    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                # ユーザー名に基づいてデータを取得
                query = f"SELECT map3d_spectrums.* FROM map3d_spectrums INNER JOIN auth_user ON map3d_spectrums.user_id = auth_user.id WHERE auth_user.username = '{username}';"
...
def create_csv_for_pattern(project, formats, owner, data_id, latitude, longitude, wavelength, reflectance):
    try:
        ...
        if owner:
            ...
            base_dir = f"/data/groups/{project}/{data_id}_{timestamp}"
    ...
    if request.method == 'POST':
        try:
            ...
            for item in data:
                ...
                result = create_csv_for_pattern(project, formats, owner, data_id, latitude, longitude, wavelength, reflectance)
