...
# JupyterHub用のディレクトリ作成関数
def create_jupyter_dir_with_check(request, parent_dir, dir_name):
    parent_dir_path = f"/data/{parent_dir}"
    dir_path = f"{parent_dir_path}/{dir_name}"
    try:
        if not os.path.exists(parent_dir_path):
            messages.error(request, f"Error: The directory {parent_dir_path} does not exist. Please access JupyterHub before creating a project.")
            return False
...
def create_project(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not create_jupyter_dir_with_check(request, 'users', request.user.username):
                return redirect('accounts/create_project.html')  # エラー時にリロード
            if not create_jupyter_dir_with_check(request, 'groups', name):
                return redirect('accounts/create_project.html')  # エラー時にリロード
        ...
        if form.is_valid():
            ...
            try:
                ...
                if project.member.filter(id=request.user.id).exists():
                    ...
                    # create_jupyter_dir('users', request.user.username)
                    if not create_jupyter_dir_with_check(request, 'users', request.user.username):
                        return redirect('accounts/join_project.html')  # エラー時にリロード
                    user_dir = f"/data/users/{request.user.username}"
                    link_path = f"{user_dir}/{name}"
                    if not os.path.exists(link_path):  # リンクが存在しない場合のみ作成
                        os.symlink(f"/data/groups/{name}", link_path)
